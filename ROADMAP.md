...
3. If dfm is launched with `-v > 1` then no progress info must be shown — progress rendering is suppressed at verbosity 2 and 3 so it cannot interleave with verbose logging. Implemented as a single gate in `ProgressLine::set` that no-ops when `log::max_level() >= LevelFilter::Info` (stderrlog maps `-v 2`→Info, `-v 3`→Debug); no separate state is needed. Covered by `tests/test_progress_indicator.sh`.
