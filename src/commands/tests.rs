...
use crate::commands::{add_command, init_command};
...
fn add_file_to_source() {
    ...
    let config_for_add =  merge_settings(&config_for_init, &config_from_file, Some(&state));
...
fn create_default_test_config(source: Option<PathBuf>, target: Option<PathBuf>) -> Settings {
    Settings {
...
fn read_config_from_test_file() -> Option<Config> {
