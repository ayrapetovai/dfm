...
fn handle_encrypted_timestamps(
    ...
    match cmp {
        CompareByTimestamp::BothModified => {
            warn!(
                "both target and encrypted source {:?} were modified, merge needed",
                source_abs
            );
            require_force(
                force,
                "target and encrypted source have conflicting modifications",
            )?;
            tasks.push(PullTask::Decrypt(
        ...
        CompareByTimestamp::NonModified => {
            if force {
                ...
                info!(
                    "neither target nor encrypted source were modified, no action needed, skipping..."
                );
            }
        }
...
fn handle_source_path(
    ...
    let source_rel_str = file_path_relative_to(source_file_abs_path, source_dir_abs_path)
        .to_string_lossy()
        .into_owned();
    ...
    if handle_ignore_or_override(
        target_ignore_regex,
        &PathBuf::from(&target_file_rel_to_target_dir),
        force,
        patterns_to_remove,
    ...
    if !target_file_abs_path.exists() && source_file_abs_path.exists() {
        if source_name.ends_with(&settings.symlink_postfix) {
            ...
            debug!(
                "source is a symlink file, pointing to {}",
                source_file_content
            );
            tasks.push(PullTask::CreateOrUpdateSymlink(
                target_file_abs_path,
                source_file_content,
            ));
            return Ok(None);
            ...
            debug!(
                "decrypting source {:?}\n\tto target {:?}",
                source_file_abs_path, target_file_abs_path
            );
            tasks.push(PullTask::Decrypt(
                target_file_abs_path,
                source_file_abs_path.to_path_buf(),
            ));
            return Ok(None);
        } else {
            if target_must_be_symlink {
                ...
                tasks.push(PullTask::CreateOrUpdateSymlink(
                    target_file_abs_path.clone(),
                    source_file_abs_path.to_string_lossy().into_owned(),
                ));
            } else {
                debug!("regular file creating task");
                tasks.push(PullTask::Copy(
                    target_file_abs_path,
                    source_file_abs_path.to_path_buf(),
                ));
            }
    ...
    } else if target_file_abs_path.is_symlink()
        && source_file_abs_path.exists()
        && source_name.ends_with(&settings.symlink_postfix)
    {
        ...
        let target_symlink_pointee =
            fs::read_link(&target_file_abs_path).map_err(|e| io_err(&target_file_abs_path, e))?;
        let source_file_content = read_symlink_pointer(source_file_abs_path)?;
        if source_file_content != target_symlink_pointee.to_string_lossy().as_ref() {
            info!(
                "target symlink {:?} points to {:?},\n\tmust point to {:?}",
                target_file_abs_path, target_symlink_pointee, source_file_content
            );
            tasks.push(PullTask::CreateOrUpdateSymlink(
                target_file_abs_path,
                source_file_content,
        ...
        debug!(
            "target {:?} exists, source is encrypted, checking timestamps",
            target_file_abs_path
        );
...
fn handle_existing_target(
    ...
    if target_abs_path.is_symlink() {
        let target_symlink_followed_abs_path = fs::canonicalize(target_abs_path).map_err(|e| {
        ...
        let source_file_abs_path = filepath_in_source_dir(
            &settings.dot_prefix,
            target_dir_abs_path,
            source_dir_abs_path,
            target_abs_path,
        ...
        if target_symlink_followed_abs_path == source_file_abs_path {
            info!(
                "target symlink {:?}\n\tpoints to the source file {:?}, skipping...",
        ...
        if source_symlink_file_abs_path.exists() {
            ...
            if !target_symlink_followed_abs_path.starts_with(source_dir_abs_path) {
                info!(
                    "target symlink {:?} does not point to the source directory, skipping...",
                    target_abs_path
                );
    ...
    let source_abs_path = filepath_in_source_dir(
        &settings.dot_prefix,
        target_dir_abs_path,
        source_dir_abs_path,
        target_abs_path,
    ...
    if !source_abs_path.exists() {
        ...
        let source_encrypted_abs_path = filepath_in_source_dir(
            &settings.dot_prefix,
            target_dir_abs_path,
            source_dir_abs_path,
            target_abs_path,
        ...
        if source_encrypted_abs_path.exists() {
            debug!(
                "target {:?} exists, encrypted source found, checking timestamps",
                target_abs_path
            );
    ...
    match cmp {
        ...
        CompareByTimestamp::NeverSynchronized => {
            ...
            error_list.push(format!(
                "target {:?} and source {:?} were not synchronized",
                target_abs_path, source_abs_path
            ));
            if !force {
                return Ok(());
...
fn handle_missing_target(
    ...
    let source_encrypted_file_abs_path = filepath_in_source_dir(
        &settings.dot_prefix,
        target_dir_abs_path,
        source_dir_abs_path,
        target_abs_path,
    ...
    if source_encrypted_file_abs_path.exists() {
        info!(
            "encrypted source {:?} will be decrypted\n\tto the target {:?}",
            source_encrypted_file_abs_path, target_abs_path
        );
        tasks.push(PullTask::Decrypt(
            target_abs_path.to_path_buf(),
            source_encrypted_file_abs_path,
        ));
        return Ok(());
    }
    ...
    if source_symlink_file_abs_path.exists() {
        info!(
            "source symlink file {:?} will be used to create a target symlink",
    ...
    Err(DfmError::NotFound(format!(
        "for target {:?} no corresponding source file found",
        target_abs_path
    )))
}
...
pub fn pull_command(
    ...
    let PullArgs {
        ref paths,
        ref force,
        symlink: ref target_must_be_symlink,
        dry_run,
    } = args;

    debug!(
        "pull paths {:?}, force {}, dry-run {}",
        paths, force, dry_run
    );
    ...
    let regex_no_dot_files = RegexSet::new(vec![PULL_KEEP_NON_DOTFILES]).map_err(|e| {
        DfmError::Other(format!(
            "invalid keep-non-dotfiles regex {:?}: {}",
            ...
            if let Err(e) = symlink::remove_symlink_file(target_symlink_file_path) {
                match e.kind() {
                    std::io::ErrorKind::NotFound => {
                        info!(
                            "target symlink {:?} does not exist",
                            target_symlink_file_path
                        );
                        // is ok
                    }
