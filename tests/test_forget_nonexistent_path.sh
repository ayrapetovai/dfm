...
run_fail dfm forget ghost.txt
assert_succ grep -qF "ghost.txt does not exist" <<<"$FAIL_OUTPUT"

# nonexistent source-side path whose target does not exist either
run_fail dfm forget dotfiles/dot_ghost.txt
assert_succ grep -qF "dotfiles/dot_ghost.txt does not exist" <<<"$FAIL_OUTPUT"

# several paths, one missing: the run fails and nothing is forgotten
write "managed" managed.txt
dfm add managed.txt
run_fail dfm forget managed.txt another_ghost.txt
assert_succ grep -qF "another_ghost.txt does not exist" <<<"$FAIL_OUTPUT"
