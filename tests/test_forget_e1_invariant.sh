...
assert -f f2.txt
assert "modified-$CONTENT2" = "$(cat f2.txt)"
...
assert -f f3.txt
assert "modified-target-$CONTENT3" = "$(cat f3.txt)"
...
dfm forget
assert -f f6.txt
assert "$CONTENT6" = "$(cat f6.txt)"
assert -f f7.txt
assert "$CONTENT7" = "$(cat f7.txt)"
